...
| 阶段2: 数据层开发 | 100% | 模型/仓储完善，含充足单测 |
| 阶段3: 游戏引擎开发 | 85% | 老虎机/状态机/会话/持久化OK；恢复细节待补 |
| 阶段4: 通信层开发 | 55% | WebSocket完备；串口已实现未集成；MQTT未做 |
| 阶段5: 业务集成 | 75% | 用户/认证/钱包/老虎机API就绪；管理端API缺失 |
| 阶段6: 监控与优化 | 10% | 仅有/health；缺指标、限流、熔断等 |
| 阶段7: 测试与部署 | 40% | 单测较全；缺集成/E2E；systemd脚本已备 |

说明:
- 已核对实际代码路径与实现，去除了不符现实的过度乐观描述与统计口径。
- 部分功能已实现但未串联到主流程（如串口控制器），以下明确为“待集成”。
...
1) 高频使用/影响面广
- [ ] WebSocket → API → 游戏 → 钱包 全链路集成回归（批量转动、结算边界）
- [ ] 游戏恢复补完：退款流程/结算确认/过期清理（见下“恢复与持久化”）
- [ ] 串口控制器集成到中奖流程（推币动作、异常回退、命令队列）

2) 安全与可运营
- [ ] 重置密码全流程（令牌存储、邮件发送、令牌校验）
- [ ] 管理后台API最小闭环（用户列表/封禁、系统统计、日志查询）
- [ ] 限流与熔断（保护关键接口：登录、转动、结算、充值/提现）

3) 观测与稳定
- [ ] Prometheus指标/pprof/结构化日志聚合
- [ ] 基础集成测试与WebSocket通道E2E
- [ ] MQTT远程控制（如需要线上管控）
...
- 恢复与持久化
  - [ ] RedisStatePersister 未实现（Save/Load/Delete 为空）
  - [ ] 恢复策略细节：Ready退款、Settlement完成性确认
  - [ ] 批量清理过期会话（需扩展持久化接口）
...
- 管理与运营
  - [ ] 管理后台API（router中均为TODO占位）
  - [ ] 指标/日志聚合/健康检查增强/告警

- 测试与交付
  - [ ] 通信模块/集成/E2E 测试
  - [ ] Swagger/OpenAPI 文档生成（Makefile 已有 swag 目标，未生成 docs/api）
